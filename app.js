class LastWarNexus {
    constructor() {
        ...
        this.data = {
            armsracephases: [
                { id: 6, name: "Server Reset", icon: "🔄", activities: ["Server maintenance", "Reset preparations"], pointSources: ["Wait for reset completion", "Prepare for new phase"] },
    ...
    cacheElements() {
        const elementIds = [
            ...
            'optimization-focus', 'time-remaining', 'priority-level', 'settings-toggle',
            'settings-dropdown', 'priority-grid', 'schedule-grid', 'intelligence-content',
            'priority-count', 'schedule-count', 'intel-count', 'event-modal', 'modal-title',
            'modal-body', 'modal-close', 'modal-share', 'modal-remind', 'time-format-dropdown',
            'detail-level-dropdown', 'view-scope-dropdown', 'bottom-priority-cards',
            'bottom-priority-grid', 'minimize-cards', 'bottom-cards-content', 'server-toggle',
            'server-dropdown', 'apply-server', 'current-server', 'current-arms-phase',
    ...
    createWeekSchedule() {
        try {
            ...
            this.data.vsdays.forEach(vsDayData => {
                ...
                // Phase cells - use corrected phase schedule
                const phaseHours = [0, 4, 8, 12, 16, 20];
                phaseHours.forEach(h => {
                    const armsPhase = this.getArmsRacePhase(h);
    ...
    getArmsRacePhase(utcHour) {
        // Corrected phase schedule - removed Mixed Phase from 00:00
        const phaseSchedule = [
            { hours: [0, 1, 2, 3], index: 2 },     // City Building (Night/Early Morning)
        ...
        return this.data.armsracephases[1]; // Default to Drone Boost
    }
    ...
    getNextHighPriorityWindow() {
        try {
            ...
            for (let dayOffset = 0; dayOffset < 7; dayOffset++) {
                ...
                // Check each high priority alignment
                this.data.highpriorityalignments.forEach(alignment => {
                    ...
                    // Find the correct phase hour for this alignment
                    phaseHours.forEach(hour => {
