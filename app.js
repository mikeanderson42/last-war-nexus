class LastWarNexus {
    constructor() {
        ...
        this.maxInitAttempts = 5; // Increased from 3
    ...
    cacheElements() {
        const elementIds = [
            'server-time', 'current-vs-day', 'arms-phase', 'alignment-indicator', 
            'alignment-status', 'vs-day-details', 'arms-race-details', 'vs-day-content', 
            'arms-race-content', 'current-vs-status', 'current-arms-status', 
            'countdown-timer', 'event-name', 'event-time', 'progress-fill', 'progress-text',
            'action-icon', 'action-text', 'strategy-rating', 'optimization-focus', 
            'time-remaining', 'priority-level', 'settings-toggle', 'settings-dropdown',
            'priority-grid', 'schedule-grid', 'intelligence-content', 'priority-count',
            'schedule-count', 'intel-count', 'event-modal', 'modal-title', 'modal-body',
            'modal-close', 'modal-share', 'modal-remind', 'time-format-dropdown',
            'detail-level-dropdown', 'view-scope-dropdown', 'bottom-priority-cards',
            'bottom-priority-grid', 'minimize-cards', 'bottom-cards-content',
        ...
        // Continue with partial initialization if only non-critical elements are missing
        const criticalElements = ['priority-grid', 'current-vs-day', 'arms-phase', 'countdown-timer'];
        const criticalMissing = missingElements.filter(id => criticalElements.includes(id));
        
        if (criticalMissing.length === 0) {
            console.log('Critical elements found, proceeding with initialization');
            return true;
        }
        ...
        try {
            ...
            if (this.expandedDetails.vsDay && this.elements['vs-day-content']) {
                ...
                const activities = this.settings.detailLevel === 'comprehensive' ? 
                    vsDayData.pointActivities : vsDayData.activities;
            ...
            if (this.expandedDetails.armsRace && this.elements['arms-race-content']) {
                ...
                const sources = this.settings.detailLevel === 'comprehensive' ? 
                    armsPhase.pointSources : armsPhase.activities;
    ...
    calculatePhaseTimeRemaining(utcHour, utcMinute) {
        try {
            ...
            const now = new Date();
            const phaseEndTime = new Date(now);
